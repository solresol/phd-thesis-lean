...
increment is `domainFieldRowComputableInPolyTime`: from a checked
source-order row `[index, |D|, ...D]`, a concrete finite machine emits every
exact `[3, 0, index, value]` block in at most `20 * (s+1)^2` steps in the
complete row wire length `s`. The proof covers empty and singleton domains,
preserves the supplied index across every occurrence, verifies the explicit
row count in the input decoder, and halts with all non-output stacks empty.
`scopeFieldBlockComputableInPolyTime` supplies the complementary intact-scope
branch.
...
actual compiler input bit length. The outer finite driver that advances the
index between domain rows, switches to the scope branch, maintains record
counts, and stages the complete output remains absent;

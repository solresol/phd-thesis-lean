...
increment is `domainOccurrenceBlockComputableInPolyTime`: from the checked
uncounted source-order field encoding of an arbitrary current index and value,
a concrete finite machine emits the exact tagged block
`[3, 0, index, value]` in at most `2s+2` steps.
...
actual compiler input bit length. The outer finite domain/scope record-staging
loop remains absent; `binaryPredComputableInPolyTime` already supplies its
empty- and singleton-safe binary countdown operation.

...
| `cor:all-different-csp` | Partial | `AllDifferentCSP.ExplicitSystem` supplies explicit syntax, nonempty-domain well-formedness, a deduplicated primal graph, and canonical rank relabelling onto \(\{1,\ldots,q\}\). `selectPrimeAbove` executably scans the Bertrand interval and proves the selected prime satisfies \(q<p<2q\) for \(q>1\), with explicit \(q=0,1\) cases. `compilerPrime_allDifferent_correctness` and `compilerPrime_globalMin_iff_satisfies_of_satisfiable` compose that choice with the exact p-adic minimum-conflict and satisfiable-case semantics. Finite output encodings, bit-size bounds, and a genuine polynomial-time theorem for the whole compiler, including prime selection, remain. |

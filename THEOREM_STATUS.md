...
increment adds the complete scope-section encoding contract and exact
section/header assembly identities in `AllDifferentCSPScopeSection.lean`.
`ScopeFieldSection.inputFinEncoding` and `rowPayloadFinEncoding` reuse the
existing counted-row parser; `scopeRowPayloadStructuredComputableInPolyTime`
reuses the concrete `2s+1`-step outer-count removal machine. The checked
`outputFinEncoding` decodes every tagged scope to exact exhaustion, preserving
empty scopes, repetitions, and all entries; `outputEncode_injective` rules
out boundary loss, while the rejection lemmas exclude absent or incorrect
tags. `outputEncode_length_le_linear` bounds the complete output by `4s` in
actual raw payload length, and `outputEncode_length_le_input_linear` states
the same bound against the complete counted input.
`StructuralFieldStream.encode_eq_header_sections` fixes the exact record and
variable headers followed by the domain and scope outputs, and
`raw_encode_eq_reversed_sections` proves that reverse staging is exactly the
checked raw structural view. These size and assembly theorems do not provide
the repeated-scope machine or executable header staging.
...
actual compiler input bit length. The scope-section loop, executable

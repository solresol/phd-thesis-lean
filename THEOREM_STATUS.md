...
| `cor:all-different-csp` | Partial | `AllDifferentCSP.ExplicitSystem` supplies explicit finite-domain syntax, nonempty-domain well-formedness, a deduplicated primal graph, and executable canonical rank relabelling onto \(\{1,\ldots,q\}\). Checked equivalences preserve shared-value equality, scope satisfaction, deduplicated minimum conflict, and satisfiability across relabelling. Prime selection, dataset emission, finite encodings, bit-size bounds, and a genuine polynomial-time compiler theorem remain. |

...
| `cor:all-different-csp` | Partial | `AllDifferentCSP.ExplicitSystem` supplies explicit syntax, nonempty-domain well-formedness, a deduplicated primal graph, and canonical rank relabelling onto \(\{1,\ldots,q\}\). `suppliedPrime_allDifferent_correctness` checks the p-adic objective and exact minimum-conflict semantics for a supplied prime \(p>q\); `suppliedPrime_globalMin_iff_satisfies_of_satisfiable` handles the satisfiable case. Compiler-selected prime construction, finite output encodings, bit-size bounds, and a genuine polynomial-time compiler theorem remain. |

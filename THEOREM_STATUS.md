...
all non-output stacks are empty at halt. Converting this computed tally to a
binary header and executing the raw/framed bridge remain open.
...
separate pass retains it while computing the unary outer row count. Converting
that tally to a binary header and bridging to the original framed Boolean encoding remain.

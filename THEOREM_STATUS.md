...
increment is `scopeFieldBlockComputableInPolyTime`: from a checked
source-order scope encoding containing its entry count and complete entries, a
concrete finite machine emits the exact intact tagged block
`[|S|+1, 1, ...S]` in at most `3s+8` steps. The general proof covers empty and
singleton scopes, increments the binary row-length field, and copies every
entry field byte-for-byte. `domainOccurrenceBlockComputableInPolyTime`
supplies the complementary local domain-occurrence branch.
...
actual compiler input bit length. The outer finite driver that applies both
local branches, maintains domain indices and record counts, and stages the
complete output remains absent; `binaryPredComputableInPolyTime` already
supplies its empty- and singleton-safe binary countdown operation.

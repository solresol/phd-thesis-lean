...
The generic `BinaryNatLists` codec is imported from `lean-np-hardness` and
re-exported here for compatibility. This module supplies the CSP-specific
runtime-sized, nondependent input and output syntax for
...
export LeanNPHardness.BinaryNatLists
  (frame encodeNat readUnary readBits decodeNatPrefix
   readUnary_replicate readBits_append decodeNatPrefix_encodeNat_append decodeNats encodeNatList
   decodeNatListPrefix decodeNats_flatMap_append decodeNatListPrefix_encode_append decodeLists encode
   decode decodeLists_flatMap_append decode_encode finEncoding natWireSize
   listWireSize wireSize frame_length encodeNat_length encodeNatList_length
   encode_length natWireSize_pos encodeNat_length_le natWireSize_le_two_mul_add_one sum_natWireSize_le
   listWireSize_le listWireSize_le_sixteen sum_listWireSize_le wireSize_le length_le_listWireSize
   length_le_wireSize sum_lengths_le_wireSize)
end BinaryNatLists
